...
async function daily() {
  ...
  const wolfTime1 = await fetch(
    `http://api.wolframalpha.com/v2/query?appid=${tokenWolf}&input=september+17+2024&output=json&ip=138.199.21.39`
  );
  const gBannerTime = await wolfTime1.json();
  const wolfTime2 = await fetch(
    `http://api.wolframalpha.com/v2/query?appid=${tokenWolf}&input=september+9+2024&output=json&ip=138.199.21.39`
  );
