...
async function daily() {
  ...
  const wolfTime1 = await fetch(
    `http://api.wolframalpha.com/v2/query?appid=${tokenWolf}&input=March+25+2025&output=json&ip=138.199.21.39`
  );
  const gBannerTime = await wolfTime1.json();
  const wolfTime2 = await fetch(
    `http://api.wolframalpha.com/v2/query?appid=${tokenWolf}&input=march+19+2025&output=json&ip=138.199.21.39`
  );
